...
class Server:
    ...
    def checkQueues(self):
        print("Q1 ", self.pendingTransactionsRequestQueue.qsize())  # waiting for these messages to be replied back to Q1
        print("Q2 ", self.pendingSendMessagesFromServerQueue.qsize())   # messages that I need to send to network Q2
        print("Q3 ", self.pendingReleaseMessagesToProcessQueue.qsize())  # release messages received that are not processed yet Q3
...
#        if (self.pendingReplyForMessagesQueue.empty() == True or self.ReplyWaiting > 0 ):  # harmeet
        if( self.pendingReleaseMessagesToProcessQueue.empty() == True or self.ReplyWaiting > 0):  # harmeet
            return

        request = self.peekQueue( self.pendingReleaseMessagesToProcessQueue )
        ...
        if ( self.checkIfThisRequestIsOnTop(message) == True ):
            ...
            self.pendingReleaseMessagesToProcessQueue.get()
...
#                    self.lock_QueueManagement.release()
            ...
            print("RELEASE received from server")
            self.lock_Counter.acquire()
            print("lock_Counter acquire")
            print("Update Time")
            self.updateCOUNTER(make_tuple(TIME))
            ...
            if (self.checkIfThisRequestIsOnTop(Message_Received) == True):
#                if(self.checkValidityOfTransaction( TRANSACTION  )):
                print("RELEASE: appending transaction")
                self.appendTransactionToLedger(TRANSACTION)
...
def main():
    ...
    ip = 'localhost'
    port = 8090
    otherServers = {'B':"localhost,8092",'C':"localhost,8094"}
    id = 1
    client_ID = 'A'
    ledger = 'ledger_serverA.txt'
